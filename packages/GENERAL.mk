...
DEFAULT_CFG=configure --with-pie --x-includes=$(OUT_INCLUDE) --x-libraries=$(OUT_LIB) --with-libtool-sysroot=$(OUT_PREFIX) --with-sysroot=$(OUT_PREFIX) --enable-malloc0returnsnull PKG_CONFIG_PATH="$(OUT_PREFIX):$(OUT_PREFIX)/pkgconfig:$(OUT_LIB)/pkgconfig:$(OUT_PREFIX)/share/pkgconfig:$(OUT_PREFIX)/usr/lib/pkgconfig" CFLAGS="$(CFLAGS)" CXXFLAGS="$(CXXFLAGS)" CPPFLAGS="$(CXXFLAGS)" LDFLAGS="$(LDFLAGS)" --build=$(GCC_HOST_PREFIX) --host=$(GCC_PREFIX) FC=$(GCC_PATH)/$(GCC_PREFIX)-gfortran FFLAGS="$(FFLAGS)" --prefix=$(OUT_PREFIX) --datadir=$(OUT_PREFIX) --includedir=$(OUT_INCLUDE) --libdir=$(OUT_LIB) --bindir=$(OUT_BIN) --disable-shared --enable-static $(CUSTOM_CFG) 
DEFAULT_MSN=echo [properties] > c.txt && echo "sys_root='$(OUT_PREFIX)'" >> c.txt && echo "pkg_config_libdir='$(OUT_LIB)/pkgconfig'" >> c.txt && echo "needs_exe_wrapper=true" >> c.txt && echo "c_args='$(CFLAGS)'.split(' ')" >> c.txt && echo "cpp_args='$(CXXFLAGS)'.split(' ')" >> c.txt && echo "c_link_args='$(LDFLAGS)'.split(' ')" >> c.txt && echo "cpp_link_args='$(LDFLAGS)'.split(' ')" >> c.txt && echo [host_machine] >> c.txt && echo "system='$(OS)'" >> c.txt && echo "cpu_family='$(CPU_FAMILY)'" >> c.txt && echo "cpu='$(CPU)'" >> c.txt && echo "endian='$(ENDIAN)'" >> c.txt && echo [binaries] >> c.txt && echo "c='$(GCC_PREFIX)-gcc'" >> c.txt && echo "cpp='$(GCC_PREFIX)-g++'" >> c.txt && echo "ar='$(GCC_PREFIX)-ar'" >> c.txt && echo "ld='$(GCC_PREFIX)-ld'" >> c.txt && echo "objcopy='$(GCC_PREFIX)-objcopy'" >> c.txt && echo "strip='$(GCC_PREFIX)-strip'" >> c.txt && echo "pkgconfig='pkg-config'"  >> c.txt && echo "windres='$(GCC_PREFIX)-windres'" >> c.txt &&   PKG_CONFIG_PATH="$(OUT_PREFIX):$(OUT_PREFIX)/pkgconfig:$(OUT_LIB)/pkgconfig:$(OUT_PREFIX)/share/pkgconfig:$(OUT_PREFIX)/usr/lib/pkgconfig" CFLAGS="$(CFLAGS)" CXXFLAGS="$(CXXFLAGS)" CPPFLAGS="$(CXXFLAGS)" LDFLAGS="$(LDFLAGS)" FC=$(GCC_PATH)/$(GCC_PREFIX)-gfortran FFLAGS="$(FFLAGS)" LIBRARY_PATH=$(OUT_LIB) meson --cross-file c.txt _build --prefix=$(OUT_PREFIX) --datadir=$(OUT_PREFIX) --includedir=$(OUT_INCLUDE) --libdir=$(OUT_LIB) --bindir=$(OUT_BIN) --default-library=static $(CUSTOM_MSN)
DEFAULT_CMK=mkdir -p build && cd build && cmake .. -DBUILD_SHARED_LIBS=false -DCMAKE_INSTALL_PREFIX="$(OUT_PREFIX)" -DCMAKE_INCLUDE_PATH="$(OUT_INCLUDE)" -DCMAKE_LIBRARY_PATH="$(OUT_LIB)" -DCMAKE_PROGRAM_PATH="$(OUT_BIN)" -DCMAKE_PREFIX_PATH="$(OUT_PREFIX)" -DCMAKE_C_FLAGS="$(CFLAGS)" -DCMAKE_CXX_FLAGS="$(CXXFLAGS)" -DCMAKE_F_FLAGS="$(FFLAGS)" -DCMAKE_EXE_LINKER_FLAGS="$(LDFLAGS)" -DCMAKE_C_COMPILER=$(GCC_PATH)/$(GCC_PREFIX)-gcc -DCMAKE_CXX_COMPILER=$(GCC_PATH)/$(GCC_PREFIX)-g++ -DCMAKE_F_COMPILER=$(GCC_PATH)/$(GCC_PREFIX)-gfortran -DCMAKE_FC_COMPILER=$(GCC_PATH)/$(GCC_PREFIX)-gfortran -DCMAKE_LINKER=$(GCC_PATH)/$(GCC_PREFIX)-ld -DCMAKE_AR=$(GCC_PATH)/$(GCC_PREFIX)-ar -DCMAKE_RANLIB=$(GCC_PATH)/$(GCC_PREFIX)-ranlib -DCMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY -DCMAKE_SKIP_BUILD_RPATH=TRUE -DCMAKE_SKIP_RPATH=TRUE $(CUSTOM_CMK) 
