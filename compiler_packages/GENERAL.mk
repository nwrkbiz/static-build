...
DEFAULT_CFG=configure --x-includes=$(OUT_INCLUDE) --x-libraries=$(OUT_LIB) --enable-malloc0returnsnull PKG_CONFIG_PATH="$(OUT_LIB)/pkgconfig:$(OUT_PREFIX)/share/pkgconfig" CFLAGS="$(CFLAGS)" CXXFLAGS="$(CXXFLAGS)" CPPFLAGS="$(CXXFLAGS)" LDFLAGS="$(LDFLAGS)" --build=$(GCC_HOST_PREFIX) --host=$(GCC_PREFIX) FC=$(GCC_PATH)/$(GCC_PREFIX)-gfortran FFLAGS="$(FFLAGS)" --prefix=$(OUT_PREFIX) --includedir=$(OUT_INCLUDE) --libdir=$(OUT_LIB) --bindir=$(OUT_BIN) --disable-shared --enable-static $(CUSTOM_CFG) 
